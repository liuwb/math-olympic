...
\begin{tikzpicture}[>=stealth,font=\small]
  \draw[->,thick] (-0.4,0) -- (7.7,0);
  \foreach \x/\label in {0/3,1/10,2/17,3/24,4/31,5/38}
    \draw (\x*1.35,0.1)--(\x*1.35,-0.1) node[below=2pt] {\label};
  \foreach \x in {0,1,2,3,4,5}
    \fill[mainblue] (\x*1.35,0) circle (2pt);
  \draw[decorate,decoration={brace,amplitude=4pt},accent]
    (0,-0.45) -- (1.35,-0.45) node[midway,below=6pt,text=accent] {相差 7};
  \draw[decorate,decoration={brace,amplitude=4pt},accent]
    (5.4,-0.45) -- (6.75,-0.45) node[midway,below=6pt,text=accent] {相差 7};
  \node[text=rulegray] at (3.4,1.0) {同余可以看成：在数轴上相隔若干个模数长度};
\end{tikzpicture}
\end{center}
...
\begin{tikzpicture}[font=\small]
  \node[draw=mainblue,fill=lightblue,rounded corners=2pt,minimum width=3.3cm,minimum height=0.72cm] (a) at (0,0) {$x\equiv 2\pmod 3$};
  \node[draw=mainblue,fill=lightblue,rounded corners=2pt,minimum width=3.3cm,minimum height=0.72cm] (b) at (4.5,0) {$x\equiv 3\pmod 5$};
  \node[draw=mainblue,fill=lightblue,rounded corners=2pt,minimum width=3.3cm,minimum height=0.72cm] (c) at (9,0) {$x\equiv 2\pmod 7$};
  \node[draw=black!55,rounded corners=2pt,minimum width=3.3cm,minimum height=1.9cm,align=center] at (0,-2.0) {候选数：\\2, 5, 8, 11, 14, 17, 20, 23, \dots};
  \node[draw=black!55,rounded corners=2pt,minimum width=3.3cm,minimum height=1.9cm,align=center] at (4.5,-2.0) {再筛：\\8, 23, 38, 53, \dots};
  \node[draw=black!55,rounded corners=2pt,minimum width=3.3cm,minimum height=1.9cm,align=center] at (9,-2.0) {最后筛：\\23, 128, \dots};
  \draw[->,thick,accent] (a) -- (0,-1.05);
  \draw[->,thick,accent] (b) -- (4.5,-1.05);
  \draw[->,thick,accent] (c) -- (9,-1.05);
  \node[text=rulegray] at (4.5,-3.45) {逐步代入法的本质：一层一层筛掉不符合余数条件的数。};
\end{tikzpicture}
\end{center}
...
\begin{tikzpicture}[scale=0.85,font=\small]
  ...
  \draw[thick] (3.65,0) -- (3.65,0.35) -- (4,0.35);
  \node[below] at (2,0) {$a$};
  \node[right] at (4,1.5) {$b$};
  \node[above left] at (2.05,1.55) {$c$};
  \node[below left] at (0,0) {$A$};
  \node[below right] at (4,0) {$B$};
  \node[above right] at (4,3) {$C$};
  \node[text=rulegray,align=center] at (6.8,1.3) {$a^2+b^2=c^2$\\直角三角形的整数边长};
\end{tikzpicture}
\end{center}
