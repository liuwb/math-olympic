...
\begin{tikzpicture}[
  ...
  cycle/.style={draw=green!45!black,line width=1.1pt,-{Stealth[length=2mm]}}
]
  \node[draw=none,fill=none,circle=false,font=\bfseries\color{mainblue}] at (5.0,3.5) {附图参考};

  \node[draw=none,fill=none,circle=false,font=\scriptsize] at (1.3,2.9) {题 2 / 题 5 / 题 7};
  \node[fill=orange!18,draw=accent] (O) at (1.3,1.1) {};
  \node (A) at (0.1,2.2) {};
  \node (B) at (0.1,0.0) {};
  \node (C) at (2.5,2.2) {};
  \node (D) at (2.5,0.0) {};
  ...
  \begin{scope}[xshift=5.2cm]
    \node[draw=none,fill=none,circle=false,font=\scriptsize] at (1.4,2.9) {题 3：$2\times 3$ 网格图};
    \foreach \x in {0,1,2,3} {
      \foreach \y in {0,1,2} {
        \node (\x\y) at (0.8*\x,0.8*\y) {};
      }
    }
  ...
  \begin{scope}[xshift=10.5cm]
    \node[draw=none,fill=none,circle=false,font=\scriptsize] at (1.8,2.9) {题 4：$2\times 4$ 网格图};
    \foreach \x in {0,1,2,3,4} {
      \foreach \y in {0,1} {\node (\x\y) at (0.8*\x,0.9*\y) {};}
    }
