...
\begin{center}
\begin{tikzpicture}[
  numbox/.style={draw=mainblue,rounded corners=2pt,minimum width=1.05cm,minimum height=0.68cm,fill=lightblue,font=\small},
  titlebox/.style={draw=accent,rounded corners=2pt,minimum width=3.1cm,minimum height=0.72cm,fill=accent!10,font=\small\bfseries}
]
  \node[titlebox] (odd) at (0,0) {奇数};
  \node[titlebox] (even) at (4.4,0) {偶数};
  \foreach \n/\y in {1/-1,3/-1.85,5/-2.7,7/-3.55,9/-4.4}
    \node[numbox] at (0,\y) {\n};
  \foreach \n/\y in {2/-1,4/-1.85,6/-2.7,8/-3.55}
    \node[numbox] at (4.4,\y) {\n};
  \node[draw=none,font=\small\color{rulegray},align=left] at (2.2,-5.2) {做配对题时，先看两类对象的数量是否匹配。};
\end{tikzpicture}
\end{center}
...
\begin{tikzpicture}[
  ...
  listbox/.style={draw=black!50,rounded corners=2pt,minimum width=2.6cm,minimum height=2.35cm,fill=white,font=\small}
]
  \node[titlebox] (r0) at (0,0) {余 0};
  \node[titlebox] (r1) at (3.6,0) {余 1};
  \node[titlebox] (r2) at (7.2,0) {余 2};
  \node[listbox,align=center,below=0.15cm of r0] {3\\6\\9};
  \node[listbox,align=center,below=0.15cm of r1] {1\\4\\7};
  \node[listbox,align=center,below=0.15cm of r2] {2\\5\\8};
  \node[draw=none,font=\small\color{rulegray}] at (3.6,-3.15) {按模分类时，先把数“装进余数盒子”再讨论组合类型。};
\end{tikzpicture}
\end{center}
...
\begin{tikzpicture}[scale=0.72]
  ...
  \draw[step=1cm,black!55,thin] (0,0) grid (4,4);
  \fill[mainblue] (0,0) circle (2.4pt);
  \fill[accent] (3,3) circle (2.4pt);
  \node[below left] at (0,0) {\small $(0,0)$};
  \node[above right] at (3,3) {\small $(3,3)$};
  \node[draw=none,font=\small\color{rulegray}] at (2,-0.75) {每走一步，就从一种颜色跳到另一种颜色。};
\end{tikzpicture}
\end{center}
