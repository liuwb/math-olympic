...
总主题： & 组合专题阶段课程 \\
已完成专题： & 错位排列、容斥原理、递推计数、图论计数、数论方法中的组合构造、抽屉原理与极值思想、染色方法与棋盘问题 \\
...
\begin{enumerate}
  ...
  \item 欧拉路径与哈密顿路径：区分“边一次”和“点一次”的图上走法；
  \item 进一步提高：状态压缩递推、生成函数初步等。
\end{enumerate}
\end{goalbox}
...
\begin{itemize}
  ...
  \item 第 7 个专题适合作为提升阶段，把覆盖、路径与不变量真正连起来；
  \item 第 8 个专题适合在图论与染色都有基础后加入；
  \item 第 9 个专题起可继续向更高阶段方法过渡。
\end{itemize}
\end{warnbox}
...
\begin{itemize}
  ...
  \item 提高训练题单：\texttt{coloring\_chessboard\_advanced\_problem\_set.tex} / \texttt{coloring\_chessboard\_advanced\_problem\_set.pdf}
  \item 周测 A 卷：\texttt{coloring\_chessboard\_weekly\_test\_A.tex} / \texttt{coloring\_chessboard\_weekly\_test\_A.pdf}
  \item 周测 B 卷：\texttt{coloring\_chessboard\_weekly\_test\_B.tex} / \texttt{coloring\_chessboard\_weekly\_test\_B.pdf}
\end{itemize}

\subsection{欧拉路径与哈密顿路径专题}
\begin{itemize}
  \item 专题目录页：\texttt{euler\_hamilton\_topic\_index.tex} / \texttt{euler\_hamilton\_topic\_index.pdf}
...
\begin{itemize}
  ...
  \item 第 16--17 周：染色方法与棋盘问题目录页 + 讲义 + 题单 + 周测 A/B；
  \item 第 18--19 周：欧拉路径与哈密顿路径目录页 + 讲义 + 题单 + 周测 A/B。
\end{itemize}
...
在目前已经整理出的七个专题之后，比较自然的后续方向包括：欧拉路径与哈密顿路径、状态压缩递推、生成函数初步，以及更系统的图论构造专题。
\end{infobox}
